...
export default function SignInSide({userid}) {
  ...
  const handleSubmit = async (event) => {
    ...
    if(data.get('email')==''||data.get('password')==''){
      setreponse('The email address or password you entered is not associated with an account')
      setLoading(false);
      handleAffiche()
    ...
    <ThemeProvider theme={theme}>
      <Grid container component="main" sx={{ height: '100vh' }}>
        ...
        backgroundColor:'hsl(3 39% 100%/.5)'
          ...
          >
            <h1>WELCOME BACK !</h1>
            <TypeWriter onInit={(typewriter)=>{typewriter.typeString("Stress Less!").pauseFor(2000).deleteAll().typeString("Sleep More ! ").
            pauseFor(2000).deleteAll().typeString("Take your daily dose of happiness with Chillin' ").start()}}/>
